...
# Seed: `r as.character(params$seed)`
...
if (rarity <= weights[1]) {
  ararity <- "Common"
} else if (rarity >= weights[1] && rarity <= weights[2]) {
  ararity <- "Uncommon"
} else {
  ararity <- "Rare"
}
...
if (Background$Other.Ability[1] != '') {
  if (grepl('You gain', Background$Other.Ability[1], ignore.case = TRUE) | grepl('Choose two', Background$Other.Ability[1], ignore.case = TRUE) && grepl('ability boost', Background$Other.Ability[1], ignore.case = TRUE)) {
    ...
    if (randomBoosts > 0) {
      if (randomBoosts > length(skillOptions)) {
        surplusBoosts <- randomBoosts - length(skillOptions)
        randomBoosts <- length(skillOptions)
        randomBoosts <- sample(skillOptions, randomBoosts, replace=F)
        
        extraSkillOptions <-  ShortSkills
        for (rb in randomBoosts) {
          extraSkillOptions <-  extraSkillOptions[extraSkillOptions != rb]
        }
        
        surplusBoosts <- sample(extraSkillOptions, surplusBoosts, replace=F)
        
        randomBoosts <- append(randomBoosts, surplusBoosts)
      } else {
        randomBoosts <- sample(skillOptions, randomBoosts, replace=F)
      }
...
langQuant <- Bonus$Intelligence[1] + bonusLang
bonusLangList <- strsplit(Ancestry$Language.Options[1],", ")[[1]]
if (langQuant > length(bonusLangList)) {
  langSurplus <- langQuant - length(bonusLangList)
  langQuant <- length(bonusLangList)
  
  extraLang <- sample(bonusLangList,langQuant, replace = FALSE) %>% paste(collapse=", ")
  
  Ancestry$Languages[1] <- paste(Ancestry$Languages[1], ", ", extraLang, ", and ", langSurplus, " additional language(s) of your choice upon reaching level 1.",  sep = "")
} else {
  langSurplus <- 0
...
## Ancestry: [`r Ancestry$Ancestry[1]`](`r Ancestry$Link[1]`)

:::: {.columns}

::: {.column width="50%"}
![](`r Ancestry$Image.1[1]`){width=100%}

`r if (Ancestry$Image.3[1] == "") "::: {.content-hidden}"`
![](`r Ancestry$Image.3[1]`){width=100%}
`r if (Ancestry$Image.3[1] == "") ":::"`
:::

::: {.column width="50%"}
`r if (Ancestry$Image.2[1] == "") "::: {.content-hidden}"`
![](`r Ancestry$Image.2[1]`){width=100%}
`r if (Ancestry$Image.2[1] == "") ":::"`

`r if (Ancestry$Image.4[1] == "") "::: {.content-hidden}"`
![](`r Ancestry$Image.4[1]`){width=100%}
`r if (Ancestry$Image.4[1] == "") ":::"`
:::
::::

`r Ancestry$Description[1]`

## Background: [`r Background$Name[1]`](`r Background$NameLink[1]`)

[`r Background$Summary[1]`](`r Background$NameLink[1]`)

**Source:** `r Background$Source`

```{r}
if (Background$FeatLink2[1] != ""){
  splitIt <- strsplit(Background$Feat[1], ", ")[[1]]
  featDisplay <- paste("**Background Feats:** ", "[", splitIt[1], "](", Background$FeatLink[1], "), ", "[", splitIt[2], "](", Background$FeatLink2[1], ")", sep = "")
} else if (Background$Feat[1] != "") {
    featDisplay <- paste("**Background Feat:** [", Background$Feat[1], "](",Background$FeatLink[1], ")", sep = "")
} else {
  featDisplay <- ""
}
```
`r if (featDisplay == "") "::: {.content-hidden}"`
`r featDisplay`
`r if (featDisplay == "") ":::"`

`r if (Background$Other.Ability[1] == "") "::: {.content-hidden}"`
**Other Background Ability:** `r Background$Other.Ability[1]`
`r if (Background$Other.Ability[1] == "") ":::"`
...
::: {.column width="25%"}
`r 10+Bonus$Dexterity[1]`

`r Ancestry$Languages[1]`
:::
::::

`r if (Ancestry$Extra[1] == "" | grepl("One additional language", Ancestry$Extra[1], ignore.case = TRUE)) "::: {.content-hidden}"`
**Extra Ability:** `r Ancestry$Extra[1]`
`r if (Ancestry$Extra[1] == "" | grepl("One additional language", Ancestry$Extra[1], ignore.case = TRUE)) ":::"`

```{r}
if (is.null(de) == FALSE) {
  kable(de, format = "html", escape = FALSE) %>% kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), position = "left", full_width = F)
}
```
...
```{r}
kable(AbilityScores, align = "c", format = "html", escape = FALSE) %>% kable_styling(bootstrap_options = c("condensed", "responsive"), position = "left", full_width = F)
```

#### Bonuses

```{r}
kable(Bonus, align = "c", format = "html", escape = FALSE) %>% kable_styling(bootstrap_options = c("condensed", "responsive"), position = "left", full_width = F)
```
...
-   Unarmed Attacks
-   Unarmored defense
-   `r weapon[1,'Name']` `r trainedBullets`
...
equipment <- equipment %>% select_if(function(x) !(all(is.na(x)) | all(x=="")))
...
**Bulk:** `r paste(currentBulk,"/",maxBulk)`

**Spare Currency:** `r gp_display`
